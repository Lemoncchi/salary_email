...
class AccountPasswordWin(tk.Toplevel):
    ...
    def setupUI(self):
        ...
        tk.Entry(row2, textvariable=self.password, width=25, show="*").pack(
            side=tk.LEFT
        )
        ...
        row3.pack(fill="x")
        tk.Button(
            row3, text="Cancel", command=self.cancel, padx=10, pady=2, width=8
        ).pack(side=tk.RIGHT, padx=10, pady=2)
        tk.Button(
            row3, text="Save", command=self.saveBT, padx=10, pady=2, width=8
        ).pack(side=tk.RIGHT, padx=10, pady=2)
    ...
    def saveBT(self):
        ...
        if len(password_text) > 0 and re.match(
            r"[0-9A-Za-z][\.-_0-9A-Za-z]*@[0-9A-Za-z]+(\.[0-9A-Za-z]+)+$", sender_text
        ):
            ...
            tk.messagebox.showinfo(
                title="success", message="Save Successfully!", parent=self
            )
            self.destroy()
        elif len(password_text) <= 0:
            tk.messagebox.showinfo(
                title="密码错误", message="密码不能为空!", parent=self
            )
        elif not re.match(
            r"[0-9A-Za-z][\.-_0-9A-Za-z]*@[0-9A-Za-z]+(\.[0-9A-Za-z]+)+$", sender_text
        ):
            tk.messagebox.showinfo(
                title="邮箱地址错误", message="请输入正确的邮箱地址!", parent=self
            )

    def cancel(self):
        res = tk.messagebox.askyesno(
            title="是否取消设置？", message="设置的内容未保存，是否退出？", parent=self
        )
        if res:
            self.destroy()
...
class SMTPPortWin(tk.Toplevel):
    ...
    def saveBT(self):
        ...
        if len(smtp_text) and len(port_text):
            ...
            tk.messagebox.showinfo(
                title="输入错误", message="请输入正确的配置!", parent=self
            )
...
class SysSettingWin(tk.Toplevel):
    ...
    def saveBT(self):
        ...
        try:
            ...
            tk.messagebox.showinfo(
                title="输入错误", message="请输入正确的数量!", parent=self
            )
            return
        else:
